...
// Memoized NoiseSVG to prevent unnecessary re-renders
...
const Blob: React.FC<BlobProps> = React.memo(
  ({
    color,
    width,
    height,
    ...
    return (
      <motion.div
        ...
        style={{
          ...
          height,
          ...position,
          willChange: "transform, opacity", // Hints for better GPU acceleration
        }}
        initial={{ opacity: 0 }}
        animate={controls}
        transition={{
...
// Define blob data outside of the component to avoid recreating it on every render
const blobsData: BlobProps[] = [
  {
    color: "#0C8CE9",
    width: "850px",
    height: "550px",
...
const AnimatedBackground: React.FC<AnimatedBackgroundProps> = ({
  ...
  // Memoize rendered blobs to avoid recalculating on every render
  const renderedBlobs = useMemo(
    () => blobsData.map((blob, index) => <Blob key={index} {...blob} />),
    []
  );
  ...
  return (
    <div
      ...
      style={{ perspective: "1000px" }} // Retain perspective for 3D effects
    >
      <AnimatePresence>{renderedBlobs}</AnimatePresence>
